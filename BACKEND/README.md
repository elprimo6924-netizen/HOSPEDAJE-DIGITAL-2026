...
El servidor se ejecutará en: `http://localhost:3306`
...
curl http://localhost:3306/api/habitaciones

# Obtener todos los clientes
curl http://localhost:3306/api/clientes
```
...
| `npm run stop:port` | Libera el puerto 3306 cerrando procesos en escucha |
| `npm run start:clean` | Libera puerto 3306 y arranca backend en modo producción |
| `npm run dev:clean` | Libera puerto 3306 y arranca backend en modo desarrollo |
| `npm run dev:health:clean` | Libera puerto 3306, valida health y arranca en desarrollo |
...
3. Abrir frontend y validar que consuma `http://localhost:3306/api`.
...
```javascript
useEffect(() => {
    fetch('http://localhost:3306/api/habitaciones')
        .then(res => res.json())
