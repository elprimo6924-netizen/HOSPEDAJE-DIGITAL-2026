...
const filePath = path.join(__dirname, 'reserva-form.html');
let content = fs.readFileSync(filePath, 'utf8');

const startMarker = '<!-- ══ CENTER FORM ════════════════════════════════════ -->';
const startIdx = content.indexOf(startMarker);

const endMarker = '</aside>';
let endIdx = content.indexOf(endMarker, startIdx);
// Find the correct </aside> for the right sidebar. 
// It comes after <!-- ══ RIGHT SIDEBAR ══════════════════════════════════ -->
const rightSidebarMarker = '<!-- ══ RIGHT SIDEBAR ══════════════════════════════════ -->';
const rightSidebarIdx = content.indexOf(rightSidebarMarker);
endIdx = content.indexOf('</aside>', rightSidebarIdx) + '</aside>'.length;

if (startIdx === -1 || rightSidebarIdx === -1) {
    console.error('Markers not found');
    process.exit(1);
}

const newContent = `<!-- ══ CENTER FORM ════════════════════════════════════ -->
...
// Also replace the script tag from reservaForm.js to reserva.js
content = content.replace('<script src="js/reservaForm.js"></script>', '<script src="js/reserva.js"></script>');
